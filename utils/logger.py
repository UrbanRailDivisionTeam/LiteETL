...
class momgo_handler(logging.Handler):
    def __init__(self, name: str) -> None:
        ...
        if name not in CLIENT["logger"].list_collection_names():
            self.collection = CLIENT["logger"].create_collection(name, timeseries=time_series_options, expireAfterSeconds=604800)
        else:
            self.collection = CLIENT["logger"][name]
