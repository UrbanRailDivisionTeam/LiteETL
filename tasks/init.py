...
def task_init(connect_data: connect_data) -> list[task]:
    ...
    task_e2 = extract_increase(
        ...
        extract_increase_data(
            ...
            source_sync_sql=read_sql(os.path.join("interested_party", "sync", "interested_party.sql")),
            source_increase_sql=read_sql(os.path.join("interested_party", "increase", "interested_party_source.sql")),
            target_table="interested_party",
            target_increase_sql=read_sql(os.path.join("interested_party", "increase", "interested_party_target.sql")),
        )
    )
    task_e3 = extract_increase(
        ...
        extract_increase_data(
            ...
            source="相关方数据库" if not DEBUG else "pgsql服务",
            source_sync_sql=read_sql(os.path.join("interested_party_entry", "sync", "interested_party_entry.sql")),
            source_increase_sql=read_sql(os.path.join("interested_party_entry", "increase", "interested_party_entry_source.sql")),
            target_table="interested_party_entry",
            target_increase_sql=read_sql(os.path.join("interested_party_entry", "increase", "interested_party_entry_target.sql")),
        )
    )
    ...
    tasks_group = []
    tasks_group.append(task_e0)
    tasks_group.append(task_e1)
    tasks_group.append(task_e2)
    tasks_group.append(task_e3)
    tasks_group.append(task_e4)
    tasks_group.append(task_e5)
    tasks_group.append(task_e6)
    tasks_group.append(task_e7)
    tasks_group.append(task_e8)
    tasks_group.append(task_e9)
    
    tasks_group.append(task_p0)
